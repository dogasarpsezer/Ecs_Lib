            ...
            for (int i = 0; i < unityEntity.entity.componentBitMask.Length; i++)
            {
                if (unityEntity.entity.componentBitMask[i])
                {
                    ...
                    foreach (var info in fieldInfo)
                    {
                        ...
                        {
                            Color newValue = (Color) EditorGUILayout.ColorField("" + info.Name,
                                (Color) info.GetValue(comp));
                            
                            info.SetValue(comp,newValue);
                        }
                        else if (info.FieldType == typeof(Gradient))
                        {
                            Gradient newValue =
                                EditorGUILayout.GradientField("" + info.Name, (Gradient) info.GetValue(comp));
                            info.SetValue(comp,newValue);
                        }
                        else if (info.FieldType== typeof(bool))
                        {
                            bool newValue = EditorGUILayout.Toggle("" + info.Name, (bool) info.GetValue(comp));
                            info.SetValue(comp,newValue);
                        }
                        else
                        {
